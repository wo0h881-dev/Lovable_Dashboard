...
// 2. 플랫폼 가중치 보정
// [수정] 기본 점수 차이를 줄여서 플랫폼 간 역전이 가능하게 함
export function platformWeight(p: Platform): number {
  if (p === "naver") return 0.3; // 0.55 -> 0.3 (네이버 독점 방지)
  if (p === "kakao") return 0.2; // 0.25 -> 0.2
  if (p === "ridi") return 0.15; // 0.12 -> 0.15 (리디 저력 반영)
  return 0.05;
}
...
/**
 * 5. 종합 랭킹용 점수 (핵심 수정)
 * - 리디의 '평가수'를 네이버의 '조회수' 급으로 가중치 상향
 * - 플랫폼 가중치(pw)의 절대 비중을 낮춤
 */
export function computeUnifiedScore(
  ...
  if (p === "ridi") {
    ...
    // 조회수가 없는 대신 평가수(commentRatio) 비중을 대폭 높임
    return (
      rs * 0.4 +
      commentRatio * 0.4 +
      deltaRatio * 0.1 +
