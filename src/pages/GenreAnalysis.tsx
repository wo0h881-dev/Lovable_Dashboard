...
export default function GenrePage() {
  ...
  const genreBarData = useMemo(() => {
    const map: Record<
      string,
      { genre: string; naver: number; kakao: number; ridi: number }
    > = {};
    for (const n of novels) {
      if (!map[n.genre]) {
        map[n.genre] = { genre: n.genre, naver: 0, kakao: 0, ridi: 0 };
      }
      map[n.genre][n.platform] += 1;
    }
    return Object.values(map).sort(
      (a, b) =>
        b.naver + b.kakao + b.ridi - (a.naver + a.kakao + a.ridi),
    );
  }, [novels]);
          ...
          <table className="w-full text-xs min-w-[680px]">
            ...
            <tbody>
              {Object.entries(
                ...
                .map(([genre, list]) => {
                  const avgRank =
                    list.reduce((s, n) => s + (n.todayRank ?? 0), 0) /
                    list.length;
                  const avgViews =
                    list.reduce((s, n) => s + n.todayViews, 0) / list.length;
                  const avgRating =
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 font-mono text-emerald-500 whitespace-nowrap">
                      ...
                      <td className="py-2.5 px-3 font-mono text-yellow-400 whitespace-nowrap">
                        ★ {avgRating.toFixed(1)}
                      </td>
                      <td className="py-2.5 px-3 font-mono text-primary">
                        {newCount}
                      </td>
                      <td className="py-2.5 px-3 font-mono text-violet-400">
                        {reEntryCount}
                      </td>
                    </tr>
                  );
          ...
          <div className="space-y-1">
            {genreNovels.slice(0, 10).map((n) => (
              ...
              >
                ...
                <span className="flex-1 text-[13px] md:text-xs font-medium line-clamp-1 min-w-0">
                  {n.title}
                </span>
